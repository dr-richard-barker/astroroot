...
function showReportSection(){
  ...
  // When opened from the AstroBotany database (?ref=), offer to write results back.
  if(COSE_REF && !$("sendDbBtn")){
    const b=document.createElement("button");
    b.id="sendDbBtn"; b.className=$("genReport").className;
    b.textContent="Send to AstroBotany DB"; b.onclick=sendResultsToDb;
    $("genReport").insertAdjacentElement("afterend", b);
  }
}
...
// ---- CoSE shared results store (same-origin IndexedDB) + write-back ----
// Same DB/store/shape as the AstroBotany database and Leaf Pigment & Size app,
// so results written here appear back in the database's inspector.
const COSE_REF = new URLSearchParams(location.search).get("ref");
function coseOpen(){ return new Promise((res,rej)=>{ const r=indexedDB.open("cose-analysis",1);
  r.onupgradeneeded=()=>{ const db=r.result; if(!db.objectStoreNames.contains("results")) db.createObjectStore("results",{keyPath:"id"}); };
  r.onsuccess=()=>res(r.result); r.onerror=()=>rej(r.error); }); }
async function cosePutResult(rec){ const db=await coseOpen(); rec.id=`${rec.ref}::${rec.tool}`;
  await new Promise((res,rej)=>{ const t=db.transaction("results","readwrite"); t.objectStore("results").put(rec);
    t.oncomplete=()=>{db.close();res();}; t.onerror=()=>rej(t.error); }); }
async function sendResultsToDb(){
  if(!lastResult||!lastResult.traits){ alert("Trace roots first."); return; }
  const t=lastResult.traits, e=lastResult.est, imageUrl=new URLSearchParams(location.search).get("image")||"";
  const metrics={ "Total root length": String(t.length), "Root tips": t.tips, "Branch points": t.branches,
    "Angle from vertical": `${(+t.angle).toFixed(1)}°`, "Scale": pxPerCm?`${pxPerCm.toFixed(1)} px/cm`:"uncalibrated (px)" };
  if(e&&e.reliable){ metrics["Est. laterals"]=`~${e.n_laterals}`; metrics["Lateral fraction"]=e.lateral_fraction; }
  try{ await cosePutResult({ ref: COSE_REF||imageUrl, imageUrl, tool:"astroroot", toolName:"AstroRoot", metrics, generatedAt:new Date().toISOString() });
    const b=$("sendDbBtn"); if(b){ b.textContent="✓ Sent to database"; setTimeout(()=>{ if(b) b.textContent="Send to AstroBotany DB"; },3000); }
